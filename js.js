...
ul.addEventListener('click', e => {
  ...
  if (action === 'info') {
    ...
    document.getElementById(
      'contactInfoP1',
    ).textContent = `Phone: ${contact.phoneNum}`;
    document.getElementById(
      'contactInfoP2',
    ).textContent = `Age: ${contact.age}`;
    document.getElementById(
      'contactInfoP3',
    ).textContent = `Email: ${contact.email}`;
    document.getElementById(
      'contactInfoP4',
    ).textContent = `Note: ${contact.freeText}`;
    document.getElementById(
      'contactInfoP5',
    ).textContent = `Added: ${contact.dateAdded.toLocaleString()}`;
  ...
  if (action === 'edit') {
    ...
    document.getElementById(
      'contactInfoP5',
    ).textContent = `Edited: ${(contact.dateAdded = new Date())}`;
...
document.getElementById('editContactBtn').onclick = e => {
  ...
  const index = parseInt(
    document.getElementById('contactId').getAttribute('data-id'),
  );

  contactList[index].name = document.getElementById('editName').value.trim();
  if (contactList[index].name === '') {
    confirm('name cannot be void');
    while (confirmed) {
      contactList[index].name = document
  ...
  if (
    contactList[index].age < 0 ||
    document.getElementById('editAge').value > 120
  ) {
    confirm('invalid age');
    while (confirmed) {
      contactList[index].age = parseInt(
  ...
  if (contactList[index].phoneNum.length != 10) {
    confirm('phone number must contain 10 digits');
    while (confirmed) {
      contactList[index].phoneNum = document
        .getElementById('editNum')
        .value.trim();
    }
  }
  for (let i = 0; i < contactList[index].phoneNum.length; i++) {
    if (
      contactList[index].phoneNum[i] < '0' ||
      contactList[index].phoneNum[i] > '9'
    ) {
      confirm('phone number must only contain numbers!');
      while (confirmed) {
        contactList[index].phoneNum = document
          .getElementById('editNum')
          .value.trim();
  ...
  contactList[index].freeText = document
    .getElementById('editFreeText')
    .value.trim();
